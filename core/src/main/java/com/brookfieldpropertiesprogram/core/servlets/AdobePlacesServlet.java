...
public class AdobePlacesServlet extends SlingSafeMethodsServlet {

    private static final long serialVersionUID = 1L;

    transient Logger LOG = LoggerFactory.getLogger(AdobePlacesServlet.class);

    @Reference
    transient PlacesService adobePlacesService;
    ...
    protected void doGet(final SlingHttpServletRequest req,
        ...
        if (StringUtils.isNotEmpty(latitude)) {
            latitude = xssApi.filterHTML(latitude);
        }
        if (StringUtils.isNotEmpty(longitude)) {
            latitude = xssApi.filterHTML(longitude);
        }
        if (StringUtils.isNotEmpty(latitude) && StringUtils.isNotEmpty(longitude)) {
            resp.getWriter().print(adobePlacesService.getPlaces(latitude, longitude, 50));
        }
